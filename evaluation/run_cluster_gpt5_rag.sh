...
for ds in afrimedqa_mcq medqa_usmle kenya_vignettes whb_stumps afrimedqa_saq; do
  python3 run_eval.py --model gpt-5 --datasets $ds --judge --rag $RAG \
    --output-dir $OUT \
    > ${OUT}/eval_gpt5_rag_${ds}.log 2>&1 &
  echo "Started $ds (PID $!)"
done

python3 run_eval.py --model gpt-5 --datasets medmcqa_mcq --max-questions 500 --judge --rag $RAG \
  --output-dir $OUT \
  > ${OUT}/eval_gpt5_rag_medmcqa_mcq.log 2>&1 &
