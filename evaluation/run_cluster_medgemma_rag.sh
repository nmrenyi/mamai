...
echo "=== STARTING MEDGEMMA 4B + RAG EVALUATIONS ==="
for ds in afrimedqa_mcq medqa_usmle kenya_vignettes whb_stumps afrimedqa_saq; do
  python3 run_eval.py --model medgemma-gguf --datasets $ds --judge --rag $RAG \
    --model-dir /lightscratch/users/yiren/models \
    --output-dir $OUT \
    > ${OUT}/eval_medgemma_rag_${ds}.log 2>&1 &
  echo "Started $ds (PID $!)"
done

python3 run_eval.py --model medgemma-gguf --datasets medmcqa_mcq --max-questions 500 --judge --rag $RAG \
  --model-dir /lightscratch/users/yiren/models \
  --output-dir $OUT \
  > ${OUT}/eval_medgemma_rag_medmcqa_mcq.log 2>&1 &
echo "Started medmcqa_mcq capped at 500 (PID $!)"

echo "=== ALL JOBS LAUNCHED, WAITING ==="
wait
echo "=== ALL DONE ==="
ls -la $OUT/medgemma-gguf/*/
