...
echo "=== STARTING MEDITRON3 8B + RAG EVALUATIONS ==="
for ds in afrimedqa_mcq medqa_usmle kenya_vignettes whb_stumps afrimedqa_saq; do
  python3 run_eval.py --model meditron3-8b --datasets $ds --judge --rag $RAG \
    --model-dir /lightscratch/users/yiren/models \
    --output-dir $OUT \
    > ${OUT}/eval_meditron3_rag_${ds}.log 2>&1 &
  echo "Started $ds (PID $!)"
done

python3 run_eval.py --model meditron3-8b --datasets medmcqa_mcq --max-questions 500 --judge --rag $RAG \
  --model-dir /lightscratch/users/yiren/models \
  --output-dir $OUT \
  > ${OUT}/eval_meditron3_rag_medmcqa_mcq.log 2>&1 &
